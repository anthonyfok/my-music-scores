          ...
          (let* ((staff-context (ly:translator-context engraver))
            ...
            (if (equal? #t jianpu-dash-note)
                (let* ((staff-space (ly:staff-symbol-staff-space grob))
                       ;; Create the original path stencil
                       (path-stencil (make-connected-path-stencil '((1 0)) 0.3 1 1 #f #f))
                       ;; Scale the stencil by the staff-space (which changes with \magnifyStaff)
                       (scaled-stencil (ly:stencil-scale path-stencil staff-space staff-space)))
                  (ly:grob-set-property! grob 'stencil
                    (ly:stencil-translate
                      scaled-stencil
                      ;; Multiply the translation coordinates by staff-space so the position scales too
                      (cons 0 (* 1 staff-space)))))
     ...
     ((flag-interface engraver grob source-engraver)
      ...
      (if (ly:grob-property-data grob 'stencil)
          (let* ((glyph-name (ly:grob-property grob 'glyph-name))
                 ...
                 (flag-one (ly:stencil-translate
                            (make-connected-path-stencil '((1.2 0)) 0.15 1 1 #f #f)
                            '(0 . 0.8)))
